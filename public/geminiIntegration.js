...
export async function run(ApiKey) {
  ...
  const prompt = `explain me these lines of code in extreme depth detail with code written above and the explaination of that line of code written below.The explannation should start with "EXPLANATION" written is green color in bold letters.There shoudld be a good spacing between the each explanation. Then write the summary of code above the file with "SUMMARY OF THE CODE" written on above with bold letters and summary written below the title with details and of 300 words in detail. There should be good spacing between summary and line bu line code explanation ${data}`;
